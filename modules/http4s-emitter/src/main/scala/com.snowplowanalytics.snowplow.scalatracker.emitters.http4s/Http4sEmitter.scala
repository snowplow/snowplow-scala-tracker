...
object Http4sEmitter {
  ...
  def build[F[_]: Concurrent: Timer](collector: EndpointParams,
                                     ...
                                     callback: Option[Callback[F]] = None): Resource[F, Emitter.Flushable[F]] =
    Resource {
      for {
  ...
  private def instance[F[_]](queue: Enqueue[F, Action]): Emitter.Flushable[F] =
    new Emitter.Flushable[F] {
  ...
  private def bufferEvents[F[_]](bufferConfig: BufferConfig): Pipe[F, Action, Request] =
    _.mapAccumulate(Buffer(bufferConfig)) {
      case (buffer, Action.Terminate | Action.Flush) =>
        Buffer(bufferConfig) -> buffer.toRequest
      case (buffer, Action.Enqueue(payload)) =>
        val next = buffer.add(payload)
        if (next.isFull)
          Buffer(bufferConfig) -> next.toRequest
        else
          next -> None
    }.collect {
