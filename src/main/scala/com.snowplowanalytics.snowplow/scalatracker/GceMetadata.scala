...
object GceMetadata {
  ...
  def getMetadata: Future[JObject] =
    getString("cpu-platform").zip(getString("hostname")).zip(getString("id"))
      ...
      .zip(getJson("tags")).zip(getString("zone")).map {
      case (((((((cpuPlatform, hostname), id), image), machineType), name), tags), zone) =>
  ...
  private def getString(path: String): Future[String] =
    Future.apply(Http(InstanceMetadataUri + path).asString.body)

  private def getJson(path: String): Future[JValue] =
    Future(parse(Http(InstanceMetadataUri + path).asString.body))
}
